(function () {
	var input = `nwesesesesewswenwswsesesenesenwnesenwse
...
// wseweeenwnesenwwwswnew`; // Expected result: 10
	...
	var moves = {
		E: coord => [coord[0], coord[1] + 1],
		W: coord => [coord[0], coord[1]  - 1],
		NE: coord => [coord[0] - 1, coord[1]],
		NW: coord => [coord[0] - 1, coord[1] - 1],
		SE: coord => [coord[0] + 1, coord[1] + 1],
		SW: coord => [coord[0] + 1, coord[1]]
	}
	...
	var base = [maxMoves, maxMoves];
	var map = [];
	
	for (var i = 0; i <= maxMoves * 2; i++) {
		map[i] = [];
		for (var j = 0; j <= maxMoves * 2; j++) {
			map[i][j] = false;
		}
	}
	...
	input.forEach(x => {
		var coord = base;
		x.forEach(y => coord = moves[y](coord));
		
		map[coord[0]][coord[1]] = !map[coord[0]][coord[1]];
	});
