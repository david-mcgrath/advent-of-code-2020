...
faded cyan bags contain 1 dotted black bag, 1 shiny maroon bag, 2 muted chartreuse bags.`;
	...
	var colours = input.reduce((a, b) => { a[b.src] = b.dst; return a; }, {});
	
	function step(col) {
		if (!colours[col]) return 0;
		if (typeof colours[col].total !== "undefined") return colours[col].total;
		
		return colours[col].map(x => x.num * (1 + step(x.col))).reduce((a, b) => a + b, 0);
	}
