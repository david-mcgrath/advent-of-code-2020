(function () {
	var input = `dark olive bags contain 2 muted brown bags, 1 mirrored tomato bag, 4 bright black bags.
...
faded cyan bags contain 1 dotted black bag, 1 shiny maroon bag, 2 muted chartreuse bags.`;
	...
	input = input.split("\n")
				 ...
				 .map(x => {
					return {
						src: x[0].replace(" bags", ""),
						dst: x[1].split(", ")
								 .filter(y => y !== "no other bags.")
								 .map(y => y.replace(".", "").replace(" bags", "").replace(" bag", ""))
								 .map(y => {
									var n = parseInt(y);
									return {
										num: n,
	...
	var colours = input.map(x => x.src)
					   .concat(input.map(x => x.dst.map(y => y.col))
									.reduce((a, b) => a.concat(b)));
	colours = colours.filter((x, i) => colours.indexOf(x) === i);
	
	colours.sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
	
	var colourMaps = input.map(x => x.dst.map(y => {
											return {
												src: x.src,
												dst: y.col
											};
	...
	var acceptedColours = colours.reduce((a, b) => { a[b] = false; return a; }, {});
	var colourQueue = ["shiny gold"];
	
	while (colourQueue.length > 0) {
		var col = colourQueue.pop();
		var mapped = colourMaps[col];
		if (mapped) {
			for (var i = 0; i < mapped.length; i++) {
				if (!acceptedColours[mapped[i]]) {
					acceptedColours[mapped[i]] = true;
					colourQueue.push(mapped[i]);
				}
			}
		}
	}
	
	return Object.keys(acceptedColours).filter(x => acceptedColours[x]).length;
})();