(function (n) {
	var input = `5 + (6 + 3 + (4 + 7 * 3 + 6 + 4 + 2) * 9) * 7 + 7
...
//	input = `((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2`
	...
	function execute(expression) {
		...
		while (expression.indexOf("(") > -1 && expression.indexOf(")") > -1) {
			expression = expression.replace(/\([^\(\)]*\)/gi, function (x) {
				return execute(x.replace("(", "").replace(")", "")).toString();
			});
		}
		
		var value = null;
		var base = "";
		var previousCommand = x => value = x;
		for (var i = 0; i < expression.length; i++) {
			var c = expression[i];
			
			switch (c) {
				case "+":
					previousCommand(+base)
					previousCommand = x => value += x;
					base = "";
					break;
				case "*":
					previousCommand(+base)
	...
	return input.map(x => execute(x)).reduce((a, b) => a + b);
})(6);