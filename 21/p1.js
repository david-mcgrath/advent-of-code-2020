...
// sqjhc mxmxvkd sbzzf (contains fish)`;
	...
	input = input
		...
		.map(x => { return { Ingredients: x[0], Allergens: x[1] }; });
	
	var allergenOptions = input
		.map(x => x.Allergens
			.map(y => [y, x.Ingredients]))
		.reduce((a, b) => a.concat(b))
		.reduce((a, b) => {
			if (a[b[0]]) {
				a[b[0]] = a[b[0]]
					.filter(x => b[1].indexOf(x) > -1);
			}
			else {
				a[b[0]] = b[1];
			}
	...
	while (changed) {
		changed = false;
		Object.keys(allergenOptions)
			.forEach(x => {
				if (allergenOptions[x].length === 1) {
					var allergen = allergenOptions[x][0];
					allergens[x] = allergen;
					delete allergenOptions[x];
					
					Object.keys(allergenOptions)
						.forEach(y => allergenOptions[y] = allergenOptions[y]
							.filter(z => z !== allergen));
	...
	var possibleAllergens = Object.keys(allergens)
		.map(x => allergens[x])
		.concat(Object.keys(allergenOptions)
			.map(x => allergenOptions[x])
			.reduce((a, b) => a.concat(b), []));
	
	return input
		.map(x => x.Ingredients)
		.reduce((a, b) => a.concat(b))
		.filter(x => possibleAllergens.indexOf(x) === -1)
		.length;
})();